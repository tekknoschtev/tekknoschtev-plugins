...
1. **Get the ticket**: Ask the user for a ticket ID or have them paste the ticket content. When the Jira MCP is available, fetch the ticket directly by ID. If the ticket was created or refined using the po-plugin skills, leverage any existing acceptance criteria and technical context.
